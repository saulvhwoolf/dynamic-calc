...
abilityChanges = {
	"Cascade White": {
		...
		"Soundproof": "Amplifier",
		"Pressure": "Neutralizing Gas",
		"Early Bird": "Refrigerate",
		"Keen Eye": "Wind Rider",
		"Cute Charm": "Gooey",
		"Sticky Hold": "Aerilate",
		"Tangled Feet": "Slush Rush",
		"Steadfast": "Colossal",
		"Hydration": "Strong Jaw",
		"Stall": "Tough Claws",
		"Leaf Guard": "Fur Coat",
		"Klutz": "Distracting",
...
// slothsandmoons fork: opponent import now supports a whole team (array of injected
// temp sets) instead of a single mon. Kept the marker + set-shape so upstream stays MIT-compatible.
var ddexTemporaryOpponentSetRefs = [];
...
// Parse one Showdown block (array of trimmed, non-empty lines; block[0] is the header).
function parseDdexOpponentMon(block) {
	if (!block || !block.length) {
		return null;
	}

	var header = String(block[0] || "");
	var atParts = header.split("@");
	var namePart = atParts[0].trim();
	var item = atParts.length > 1 ? atParts.slice(1).join("@").trim() : "";

	var speciesName = findImportedSpeciesNameFromHeader(namePart);
	if (!speciesName || !pokedex[speciesName]) {
		return null;
	}

	var nickname = "";
	var pIdx = namePart.indexOf("(");
	if (pIdx > 0) {
		nickname = namePart.slice(0, pIdx).trim();
	}
...
// Parse a full opponent team from the marker onward. The marker line's trailing text is
// an optional trainer name; each blank-line-separated block after it is one mon.
function parseDdexTemporaryOpponentImport(text) {
	...
	var lines = String(text || "")
		.slice(markerIndex + DDEX_TEMP_OPPONENT_MARKER.length)
		.split(/\r?\n/);
	var trainerName = (lines.shift() || "").trim();
	...
	for (var b = 0; b < blocks.length; b++) {
		var mon = parseDdexOpponentMon(blocks[b]);
		if (mon) {
			mons.push(mon);
		}
	}
	...
	return { trainerName: trainerName || "Opponent", mons: mons };
}

function removeDdexTemporaryOpponentSet() {
	if (!Array.isArray(ddexTemporaryOpponentSetRefs) || !setdex) {
		ddexTemporaryOpponentSetRefs = [];
		return;
	}

	for (var i = 0; i < ddexTemporaryOpponentSetRefs.length; i++) {
		var ref = ddexTemporaryOpponentSetRefs[i];
		if (!ref) continue;
...
// Inject the whole opponent team into setdex so every mon is selectable on the right
// (getSetOptions reads setdex live); auto-select the ace/lead. All injected sets are
// tracked for cleanup when a non-temporary opponent is later chosen.
function importDdexTemporaryOpponent(text) {
	var team = parseDdexTemporaryOpponentImport(text);
	if (!team || !team.mons.length) {
	...
	var tr_id = "ddex_temp_" + String(team.trainerName).replace(/[^a-z0-9]/gi, "").toLowerCase();
	var refs = [];
	var firstSpecies = null;
	...
	for (var i = 0; i < team.mons.length; i++) {
		...
		setdex[mon.speciesName][setName] = buildDdexTemporaryOpponentSet(mon, { sub_index: i, tr_id: tr_id });
		refs.push({ speciesName: mon.speciesName, setName: setName });
		if (i === 0) {
			firstSpecies = mon.speciesName;
			firstSetName = setName;
		}
	}

	ddexTemporaryOpponentSetRefs = refs;
	if (typeof get_trainer_names === "function") {
		try { TR_NAMES = get_trainer_names(); } catch (e) {}
	}

	selectDdexTemporaryOpponent(firstSpecies, firstSetName);
	return true;
}
...
function addSets(pokes, name, importOptions) {
	if (isDdexTemporaryOpponentImportText(pokes)) {
		// One paste can carry both sides: everything before the marker is the player's
		// box (imported normally, left), everything after is the opponent team (right).
		var markerIdx = pokes.indexOf(DDEX_TEMP_OPPONENT_MARKER);
		var boxText = pokes.slice(0, markerIdx).trim();
		if (boxText && looksLikeImportableShowdownText(boxText)) {
			addSets(boxText, name, importOptions);
		}
